...
final readonly class AnchorNavigation implements _\ComponentInterface
{
    ...
    {
        return ($this->isSticky ? '<section class="sticky top-header z-40 h-sticky-nav-height bg-brand-grey col-span-full grid grid-cols-subgrid"><sticky-navigation data-component="AnchorNavigation" class="' . _\Util::joinAttributeValues(['col-span-content-full no-undefined-hide sticky-nav-active overflow-x-hidden flex gap-16 md:gap-24', '[--anchor-navigation-gap:var(--spacing-16)] md:[--anchor-navigation-gap:var(--spacing-24)]']) . '">' . (($temp = $this->items) === null ? '' : $temp->render()) . '</sticky-navigation></section>' : '<div class="col-span-full flex flex-wrap justify-center items-center divide-x divide-brand-grey">' . (($temp = $this->items) === null ? '' : $temp->render()) . '</div>');
    }
}
