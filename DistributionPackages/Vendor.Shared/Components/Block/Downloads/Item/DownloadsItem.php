...
final readonly class DownloadsItem implements _\ComponentInterface
{
    ...
    {
        return '<' . ($_119_tag = (((!$this->inBackend) && ($this->link->href !== null)) ? 'a' : 'div')) . ' data-component="DownloadsItem"' . ((!$this->inBackend) ? (($temp = $this->link->href) === null ? '' : ' href="' . _\Util::escapeAttributeValue($temp) . '"') : '') . '' . (($temp = $this->link->title) === null ? '' : ' title="' . _\Util::escapeAttributeValue($temp) . '"') . '' . ((!$this->inBackend) ? (($temp = $this->link->rel) === null ? '' : ' rel="' . _\Util::escapeAttributeValue($temp) . '"') : '') . '' . ((!$this->inBackend) ? (($temp = $this->link->target) === null ? '' : ' target="' . _\Util::escapeAttributeValue($temp->value) . '"') : '') . ' class="' . _\Util::joinAttributeValues(['p-16 flex flex-col justify-between h-full gap-16 bg-brand-grey', (((!$this->inBackend) && ($this->link->href !== null)) ? 'cursor-pointer hover:text-highlight' : '')]) . '"><div class="flex gap-24 h-full items-stretch"><div class="w-1/4 aspect-3/4 flex items-start shrink-0 h-full shadow-md">' . (($temp = $this->media) === null ? '' : $temp->render()) . '</div><div class="flex flex-col justify-between h-full w-full"><div class="h-full flex flex-col justify-between mb-2">' . (($temp = $this->headline) === null ? '' : $temp->render()) . '<div class="flex gap-8 justify-between mt-auto copy-medium"><span>' . (($temp = $this->primaryMetaHeadline) === null ? '' : $temp->render()) . '</span><span>' . (($temp = $this->secondaryMetaHeadline) === null ? '' : $temp->render()) . '</span></div></div></div></div></' . $_119_tag . '>';
    }
}
