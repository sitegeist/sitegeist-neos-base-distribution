...
final class LinkedButtonFactory extends AbstractComponentPresentationObjectFactory
{
    ...
    public function tryForLinkMixin(Node $node, ContentSubgraphInterface $subgraph, bool $inBackend): ?SlotInterface
    {
        ...
        $linkLabel = self::getStringValue($node, 'link__label');
    ...
    private function resolveLink(ArchaeopteryxLink $link, ContentSubgraphInterface $subgraph): ?ArchaeopteryxLink
    {
        switch ($link->href->getScheme()) {
            case 'asset':
                ...
                return $asset instanceof Asset
                    ...
                    : null;
            case 'node':
                $node = $subgraph->findNodeById(NodeAggregateId::fromString($link->href->getHost()));
